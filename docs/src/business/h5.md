...
1. 未登录活动提示（获取活动详情 / 报名活动接口的报错msg）
2. 按钮防重点击
3. 页面初始化效果，在内容过多时，避免元素闪现/消失，考虑根据获取详情接口加[遮罩层](https://vant-ui.github.io/vant/v2/#/zh-CN/overlay)
...
{
    path: 'xx',
    component: resolve => require(['@/xx'], resolve),
    meta: {
        title: 'xx',
        navBarColor: '#fff'
    }
...
> 适用于 app-h5，路由组件内的 `GET_WEB_PAGE_TITLE` 也需要额外处理
...
{
    path: 'indexs',
    component: () => import('@/views/help/indexs'),
    meta: {
        title: '示例标题',
...
对于内部 `van-popup` 的弹窗，添加 `safe-area-inset-bottom` 仍可以使底部有正常的安全距离
...
- 提供
  - 底部包含小黑条在内的一戳距离会被“隔离”，表现为白色背景
  - 整体页面/在页面底部通过 `position: fixed; bottom: 0` 添加的元素，会被顶到隔离线上边。故即使h5不添加底部安全距离处理，也不会有问题
  - 弹窗所带的遮罩层，也不能透过隔离线，会有分层现象
  - 即便在h5添加了底部安全距离处理，也不会出现额外的效果
- 不提供
  - 底部的小黑条仍然出现，背景透明，小黑条会随着背后页面的颜色内容改变而改变颜色
  - 隔离线仿佛被移到了手机的最末端，页面/底部固定按钮可以置底
  - 弹窗所带的遮罩层，可以覆盖全屏
  - 对底部固定按钮添加底部安全距离，可以让其稍微往上，看起来更合理；但注意不要让这个修改遮挡了页面元素的展示，要考虑是否对其也做上安全距离处理
...
```css
.tool {
    box-sizing: content-box; /* 设置为border-box会导致底部padding不生效 */
    position: fixed;
    bottom: 0;
    left: 0;
    z-index: 1000;
    width: 100%;
    height: 96px; /* 除底部安全距离外，高度为96px */
    /* 用padding可以让底部安全距离“上色” */
...
>  这样可以实现在页面上渲染的是实际dom，但在微信小程序仍然可以长按唤起菜单的效果

```html
<template>
    <van-overlay :show="value">
        <div class="img-wrap">
            <div class="pop-main" ref="shareContentWrap">
                <div class="inner-card">内容示例</div>
            </div>
            <img class="img-entity" :src="shareImgUrl" />
        </div>
    </van-overlay>
</template>
...
> 插件生成的二维码可以设置宽高，但单位是 `px`，可以稍加处理以适配 `rem`

```html
<div id="share-code-wrap"></div>
...
export default {
    methods: {
        createQrcode() {
            // 这里以750px的设计图画布中，宽/高为110px的二维码为例
            const long = document.documentElement.clientWidth * (110 / 750)
            new QRCode('share-code-wrap', {
                text: '分享内容/链接',
                width: long,
                height: long
            })
...
> 情景：正在支付时，如果用户尝试关闭弹窗/退出页面，执行拦截程序
...
export default {
	mounted() {
        history.pushState(null, null, document.URL)
        window.addEventListener('popstate', this.backPage)
    },
    // 一定要在组件销毁前手动销毁，否则其它页面的popstate事件也会执行该回调！
    beforeDestroy() {
        window.removeEventListener('popstate', this.backPage)
    },
    // 当前页面后续会跳转到其它页面，这时不是触发beforeDestroy
    // 如果不加处理，且后面的页面也监听popstate执行回调，会出bug
    activated() {
        window.addEventListener('popstate', this.backPage)
    },
    deactivated() {
    ...
    methods: {
        backPage() {
            if (this.showKeyboard) { // 正在支付
                history.pushState(null, null, document.URL)
                this.treatClose() // 支付退出拦截
            } else {
                history.go(-1)
            }
        }
...
> 固定二维码海报（分享链接固定，所以二维码之前由UI嵌到图片上）、保存图片、微信好友、朋友圈

<span style="padding: 1px 4px; background: #d7d7d7; color: #fff; border-radius: 2px;">app-h5</span>
...
> 非固定二维码海报（分享链接不固定，二维码需要手动生成放到海报上）、保存图片、微信好友、朋友圈

`链接`

```
http://fhd.local.lbdj.net:9500/act/applianceFirstOrder
```

`组件`

```
src\views\activity\2024\applianceFirstOrder\components\sharePop.vue
```

> 在微信小程序准备一个特殊页面，用活动页面作为背景图，写一个分享弹窗；在h5点击分享时跳转到该页面，如果关闭分享弹窗，则退回h5

`链接`

```
http://fhd.local.lbdj.net:9500/act/redEnvelope?id=64
```

`组件`

```
src\views\activity\goldenEgg\index.vue
```
...
default export {
    data() {
        return {
            hadShareToApp: false // 是否分享到其它应用
        }
    },
    mounted() {
        document.addEventListener("visibilitychange", this.judgePopShow)
    },
    beforeDestroy() {
        document.removeEventListener("visibilitychange", this.judgePopShow)
    },
	methods: {
        // 分享到微信/朋友圈/QQ
        mockShare() {
            // ...
            this.hadShareToApp = true
            // ...
        },
        judgePopShow() {
            if (document.hidden && this.hadShareToApp) {
                this.hadShareToApp = false
                this.$emit('close')
            }
        }
    }
...
> 原本使用 `window.history.length > 1` 判断是否返回上一个 h5，但在 APP 中，`A > B > C > 回退B > 返回` 时会无反应。
...
pageBack() {
    if (this.isMiniwx) {
        ...
        if (p == 29) {
            const turl = `/pages/index/index`
            this.$goMiniPage(turl, false, true)
            return
        }
		// 需要先前页面添加 goBack=1 来区分是否 h5 页面
...
① 如已登录并入驻，则走业务逻辑 `doSome`

② 如未登录，则跳转到登录页面。登录成功后，返回此落地页

③ 如未入驻，则跳转到入驻页面。注册成功后，返回此落地页

> 师傅APP那边，未登录/未入驻无法来到活动页（push消息等方式也会被拦截），所以无需额外判断

```html
<script>
import { queryWorkerInfo } from '@/api/mine.js'
...
export default {
    ...
    methods: {
        // 查询师傅是否入驻
        getQueryWorkerInfo() {
            queryWorkerInfo({}).then(res => {
                if (res.code == 200 && res.data) {
                    if (res.data.isEnter == 2) {
                        this.isEnter = 1
                    }
                }
            })
        ...
        handleSignUp() {
            if (!this.isMiniwx) {
                this.doSome() // todoMaster 业务逻辑
                return
            }

            // todoMaster 填写具体的活动路径
            let h5Url = encodeURIComponent(`${window.location.origin}/act/demo`)
            let url = '/pages/index/web-view/index?url=' + h5Url
            ...
            this.getAppToken()
                .then(res => {
                    ...
                    if (this.isEnter == 1) {
                        ...
                        // 未入驻
                        this.$goMiniPage(
                            `/packageA/pages/index/register/register-flow/register-flow?redirect=${redirect}`
                        )
                    }
                })
                .catch(() => {
                    // 未登录
                    this.$goMiniPage(`/pages/login/login?redirect=${redirect}`)
                })
    	}
	},
}
...
{
    path: 'demo',
    component: () => import('@/views/activity/demo/index'),
    meta: {
        title: 'demo',
        navBarColor: '#fff'
    }
...
<template>
    <div class="ac-page">
        <div v-if="!isMiniwx" class="custom-header">
            <div class="header-nav"></div>
            <div class="top-bar-row">
                <img
                    class="rollback"
                    src="https://xx/back.png"
                    @click="pageBack"
                />
            </div>
        </div>
        <!-- 原切图只适配了APP头会有点空，微信小程序无法自定义头部，故背景图要上去一点 -->
        <div class="header-bg" :class="{ 'shrunk-header-bg': isMiniwx }"></div>
    </div>
</template>
...
export default {
    mounted() {
        const sendObj = {
            reqType: 'GET_WEB_PAGE_TITLE',
            data: {
                iphoneHiddenStatus: 3 // 1 隐藏头部；2 隐藏底部；3 隐藏头部底部
            }
        }
        window.sendAppData(sendObj)
    }
...
.ac-page {
    ...
    width: 100%;
    min-height: 100vh;
    padding-bottom: calc(57px + constant(safe-area-inset-bottom)); /* 兼容 iOS < 11.2 */
    padding-bottom: calc(57px + env(safe-area-inset-bottom)); /* 兼容 iOS >= 11.2 */
...
.header-bg {
    height: 950px;
    background-image: url('https://xx/header.png');
...
- src\views\order\details\components\newDetails\bottomBtn.vue

| 组件               | 风格             |
| ------------------ | ---------------- |
| 带标题 单/双按钮   | titleDialog      |
| 不带标题 单/双按钮 | noTitleDialog    |
| 平铺单按钮         | tileButtonDialog |
...
<div class="image-value">
    <van-image
        v-for="(imgStr, index) in picList || []"
        :key="index"
        fit="cover"
        :src="imgStr"
        class="v-img"
        @click="showImgPreview(index, picList || [])"
    />
</div>

<!--预览图片-->
<van-image-preview
    v-model="previewConfig.show"
    get-container="body"
    :images="previewConfig.imgList"
    :start-position="previewConfig.imgIndex"
    :closeable="true"
/>
...
skipToOrder() {
    if (this.isMiniwx) {
        this.$goMiniPage(`/pages/order/order-list/order-list?activeIndex=0`)
    } else {
        const sendObj = {
            reqType: 'TO_PAGE',
            data: {
...
```javascript
if (this.isMiniwx) {
    this.$goMiniPage('/pages/order/order-list/order-list?activeIndex=0&initPromotionId=10')
} else {
    const sendObj = {
        reqType: 'TO_PAGE',
        data: {
...
```javascript
if (this.isMiniwx) {
    this.$goMiniPage('/pages/order/order-list/order-list?activeIndex=1')
} else {
    const sendObj = {
        reqType: 'TO_PAGE',
        data: {
...
skipToFinish() {
    if (this.isMiniwx) {
        this.$goMiniPage('/pages/order/orderResultNew/orderResultNew')
    } else {
        const sendObj = {
            reqType: 'TO_PAGE',
            data: {
...
skipToShare() {
    // 留意 goBack=1
    this.$router.push('/act/xxx?goBack=1')
}
```
